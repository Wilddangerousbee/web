...
func home(w http.ResponseWriter, r *http.Request) {
	...
	flLogin := requestLogin{FlLogin: false, FlPassword: false}
	...
	// 	if login == nameDB[i] && password == passwordDB[i] {
	// 		flLogin.FlLogin = true
	// 		for _, v := range userInfoId {
	// 			if v == idDB[i] {
	// 				flLogin.FlPassword = true
	// 			}
	// 		}
	// 	}
	// }

	reqJSON, _ := json.Marshal(flLogin)
...
func startServer() {
	...
	http.HandleFunc("/config", func(w http.ResponseWriter, r *http.Request) {
		...
		square, mail := strings.Split(string(body), " ")[0], strings.Split(string(body), " ")[1]
		fmt.Println(square, mail)
		...
		if err := 5; err == 4 {
			...
			floatSquare, _ := strconv.ParseFloat(square, 10)
			var res = updatePerfectCombination(floatSquare)
			...
			currentCombination, _ = json.Marshal(res)
			w.Write([]byte(resJSON))
		}
		fmt.Println("конец")
	})

	http.HandleFunc("/getCur", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Access-Control-Allow-Origin", "*")
		w.Header().Set("Access-Control-Allow-Methods", "POST, GET, OPTIONS, PUT, DELETE")
		w.Header().Set("Access-Control-Allow-Headers", "Accept, Content-Type, Content-Length, Authorization")

		fmt.Println("вот", []byte(currentCombination))

		w.Write([]byte(currentCombination))
	})
...
// 		resultJason, _ := json.Marshal(res)
// 		w.Write(resultJason)
// 	}
